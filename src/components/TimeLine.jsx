...
function TimeLine({ userProfile }) {
  const { fullname, username, posts } = userProfile;
  return (
      ...
      <div className="px-2 py-2 flex flex-col gap-2 border-b border-gray-200">
        ...
        <div className="flex ">
          ...
          <form className="w-[100%] px-4 flex flex-col items-end">
            ...
            />
            <button className="bg-blue-700 px-3 py-[2px] rounded-2xl text-gray-100">
              Tweet
            </button>
          </form>
        </div>
      </div>
      <Posts fullname={fullname} username={username} posts={posts}></Posts>
    </div>
  );
}
